---
title: "Policy Evaluation Project Guide with R"
output:
  pdf_document:
    highlight: tango
    toc: true
...
As a final step, let's adjust our expendature variable for inflation. To do so, we'll use the `blscrapeR` package with allows us to directly download Cunsumer Price Index (CPI) data which we need to adjust for inflation:
...
It's great that we have the DiD estimate, but if we want to conduct a hypothesis test we'll need to calculate its standard errors. The eaisest way to do this is to run a regression. The specification we need to use is this:
...
Check out the estimate for our interaction `treatment_period:treatment_group`! Exactly what we calculated. Unforunatly, however, the treatment doesn't seem to have any effect. Let's add this model to our results table.
