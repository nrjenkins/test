...
To start, we need to load `rstan` which will allow us to interface with Stan through R.
...
linear.fit.1 <- sampling(object = linear.model,
                         data = model.data)
...
linear.fit.2 <- sampling(object = linear.model, data = model.data)
...
linear.fit.3 <- sampling(object = linear.model, data = model.data)
...
```{r}
linear.fit.4 <- sampling(linear.model, data = model.data)
print(linear.fit.4)
```
